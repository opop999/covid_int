...
# Package names
packages <- c("dplyr", "tidyr", "forcats", "fpc", "NbClust", "factoextra", "cluster", "clValid", "mice", "miclust", "plotly")
...
optimal_nr_of_clusters <- readRDS("data/nr_clust.rds") %>% bind_rows()
```
...
list_clustered_dfs[[i]] <- list_imp_dfs_no_fct[[i]] %>%
  scale() %>%
  as_tibble() %>%
...
pooled_df <- tibble(variable = list_cl_dfs[[1]][[2]],
...
(ggplot(pooled_df, aes(x = variable, y = value, group = as.factor(cluster), colour = as.factor(cluster))) +
