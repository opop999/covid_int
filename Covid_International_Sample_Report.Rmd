...
### Visuals {.tabset .tabset-pills}

#### Depression by gender and age group


```{r}
data %>% 
  filter(q01_gender != "other", q02_age_group != "NA") %>% 
  ggplot(aes(y = PHQ8, x = q01_gender)) +
         geom_boxplot(na.rm = TRUE, aes(color = q01_gender)) +
         theme_bw() +
         theme(legend.position = "none") +
         facet_grid(cols = vars(q02_age_group)) + 
         scale_color_npg()
```
#### Age and depression by gender
...
data %>% 
  ...
  ggplot(aes(x = q02_age, y = PHQ8)) +
         geom_jitter(na.rm	= TRUE, size = 1, alpha = 0.1) +
         stat_smooth(method = "gam", formula = y ~ x, na.rm	= TRUE, aes(color = q01_gender)) +
         theme_bw() +
         scale_x_continuous(limits = c(16, 92), 
                     breaks = seq(from = 16, to = 92, by = 8)) + 
         scale_color_npg() 
```

#### Depression by gender and relationship type
```{r}
# For reproducibility of results
set.seed(2021)

data %>% 
  filter(q01_gender != "other", q03_relationship_type != "NA") %>% 
  ggplot(aes(y = PHQ8, x = q01_gender)) +
         geom_boxplot(na.rm = TRUE, aes(color = q01_gender)) +
         theme_bw() +
         theme(legend.position = "none") +
         ...
         scale_color_npg()
```
#### Depression by gender and education attainment
```{r}
# For reproducibility of results
set.seed(2021)

data %>%
  filter(q01_gender != "other", q11_education != "NA") %>% 
  ggplot(aes(y = PHQ8, x = q01_gender)) +
         geom_boxplot(na.rm = TRUE, aes(color = q01_gender)) +
         theme_bw() +
         theme(legend.position = "none") +
...
#### Depression by gender and children
```{r}
# For reproducibility of results
set.seed(2021)
...
#### Depression by gender and SRH
```{r}
# For reproducibility of results
set.seed(2021)

data %>%
  filter(q01_gender != "other", q47_self_reporting_health != "NA") %>% 
  ggplot(aes(y = PHQ8, x = q01_gender)) +
         geom_boxplot(na.rm = TRUE, aes(color = q01_gender)) +
         theme_bw() +
         theme(legend.position = "none") +
         facet_grid(cols = vars(q47_self_reporting_health)) + 
         scale_color_npg()
```
...
pam9 <- pam(data_scaled, 9)
print(pam9)
pam2 <- pam(data_scaled, 2)
print(pam2)

#probamos algunas visualizaciones
fviz_cluster(pam2, data = data_scaled, ellipse.type = "norm")
fviz_cluster(pam9, data = data_scaled)

res9 <- hcut(data_scaled, k = 9, stand = TRUE, method = "median")
...
data_scaled <- USArrests
data_scaled
data_scaled <- scale(USArrests)
data_scaled
data_scaled <- as.data.frame(df)
data_scaled
data_scaled$clus <- as.factor(pam9$clustering)
data_scaled

data_scaled$clus <- factor(data_scaled$clus)
data_long <- gather(data_scaled, caracteristica, valor, Murder:Rape, factor_key = TRUE)
data_long
